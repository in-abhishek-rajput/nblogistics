...
class UserRepository
{
    ...
    public function filter($params)
    {
        ...
        // $this->model = $this->model->when(!empty($params['mobile']), function ($query) use ($params) {
        //     $query->whereNotIn('mobile', $params['mobile']);
        // });
        ...
        $this->model = $this->model->whereHas('roles', function ($query) {
            $query->whereNotIn('name', config('constants.SKIP_ROLES'));
        });
        // dd(app('common')->getFormattedQuery($this->model));
        if(isset($params['return_type']) && $params['return_type'] == 'model'){
            return $this->model;
        }
        if(isset($params['return_type']) && $params['return_type'] == 'object'){
