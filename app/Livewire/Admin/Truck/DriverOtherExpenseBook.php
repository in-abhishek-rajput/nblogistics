...
class DriverOtherExpenseBook extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    protected $listeners = [
        'openDriverExpenseBookPanel' => 'openPanel',
        'editDriverExpense' => 'editDriverExpense',
        'deleteDriverExpense' => 'deleteDriverExpense',
    ];
    ...
    public function showAddExpenseModal(): void
    {
        $this->resetForm();
        $this->payment_mode = 'cash';
        $this->expense_date = now()->format('Y-m-d');
        $this->dispatch('showAddDriverExpenseModal');
    }
    ...
    {
        $expense = TripExpense::where('truck_id', $this->truckId)
    ...
    public function deleteDriverExpense(int $expenseId): void
    {
        ...
        $this->dispatch('driverExpenseBookUpdated');
        $this->dispatch('openDriverExpenseBookOffcanvas');
        $this->refreshData();
    }
    ...
    {
        return [
            'cost' => (float) $this->expensesQuery()->sum('amount'),
        ];
    }
    ...
    {
        $defaults = ['Driver Payment', 'Toll', 'Food', 'Parking', 'Helper Charges', 'Advance', 'Other'];
        $existing = TripExpense::where('expense_category', 'truck')
    ...
    {
        $query = TripExpense::where('truck_id', $this->truckId)
            ->where('expense_category', 'truck');
    ...
    protected function storeExpenseImage(?string $existingPath = null): ?string
    {
        if ($this->expense_image) {
            return $this->expense_image->store('trucks/driver-expenses', 'public');
        }
    ...
    {
        $this->dispatch('driverExpenseBookUpdated');
        $this->dispatch('closeModal', 'addDriverExpenseModal');
        $this->dispatch('openDriverExpenseBookOffcanvas');
        $this->refreshData();
    }
    ...
    {
        return view('livewire.admin.truck.driver-other-expense-book', [
