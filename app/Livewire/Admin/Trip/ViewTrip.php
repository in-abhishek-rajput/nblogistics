...
class ViewTrip extends Component
{
    ...
    {
        $this->reloadTrip(); // Ensure we have the latest data
        ...
        $this->reloadTrip(); // Reload to reflect the update
    }
    ...
    {
        try {
            $advance = $this->trip->advances()->findOrFail($advanceId);
            $advance->delete();
    ...
    {
        try {
            $charge = $this->trip->charges()->findOrFail($chargeId);
            $charge->delete();
    ...
    {
        try {
            $payment = $this->trip->payments()->findOrFail($paymentId);
            $payment->delete();
