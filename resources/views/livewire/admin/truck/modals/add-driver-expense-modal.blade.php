    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            <div class="modal-header border-bottom-0">
                <h5 class="modal-title fw-bold" id="addDriverExpenseModalLabel">
                    {{ $editingExpenseId ? 'Edit Expense' : 'Add Entry' }}
                </h5>
            ...
            <form wire:submit.prevent="{{ $editingExpenseId ? 'updateExpense' : 'createExpense' }}">
                        ...
                        <div class="d-flex flex-column gap-2">
                            @foreach ($paymentModeOptions as $key => $label)
                                <label class="btn btn-outline-secondary d-flex align-items-center gap-2">
                                    <input type="radio" wire:model.live="payment_mode" name="driver_payment_mode" value="{{ $key }}" class="form-check-input mt-0" />
                                    <span>{{ $label }}</span>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="modal-footer border-top-0 pt-0 pb-4 px-4">
                    ...
                    <button type="submit" class="btn btn-primary">{{ $editingExpenseId ? 'Update Expense' : 'Save Expense' }}</button>
                </div>
            </form>
        </div>
    </div>
